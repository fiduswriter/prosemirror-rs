...
describe("ContentMatch", () => {
    describe("matchType", () => {
        ...
        it("accepts a nested repeat", () => valid("(heading paragraph+)+", "heading paragraph heading paragraph paragraph"));
        it("fails on extra input after a nested repeat", () => invalid("(heading paragraph+)+", "heading paragraph heading paragraph paragraph horizontal_rule"));
        ...
        it("rejects a sequence with too many elements", () => invalid("hard_break{2, 4}", "hard_break hard_break hard_break hard_break hard_break"));
    ...
    describe("fillBefore", () => {
        it("returns the empty fragment when things match", () => fill("paragraph horizontal_rule paragraph", doc(p(), hr()), doc(p()), doc()));
        it("adds a node when necessary", () => fill("paragraph horizontal_rule paragraph", doc(p()), doc(p()), doc(hr())));
        ...
        it("completes a sequence", () => fill3("paragraph horizontal_rule paragraph horizontal_rule paragraph", doc(p()), doc(p()), doc(p()), doc(hr()), doc(hr())));
        it("accepts plus across two bounds", () => fill3("code_block+ paragraph+", doc(pre()), doc(pre()), doc(p()), doc(), doc()));
        it("fills a plus from empty input", () => fill3("code_block+ paragraph+", doc(), doc(), doc(), doc(), doc(pre(), p())));
        it("completes a count", () => fill3("code_block{3} paragraph{3}", doc(pre()), doc(p()), doc(), doc(pre(), pre()), doc(p(), p())));
        it("fails on non-matching elements", () => fill3("paragraph*", doc(p()), doc(pre()), doc(p()), null));
        it("completes a plus across two bounds", () => fill3("paragraph{4}", doc(p()), doc(p()), doc(p()), doc(), doc(p())));
        it("refuses to complete an overflown count across two bounds", () => fill3("paragraph{2}", doc(p()), doc(p()), doc(p()), null));
    });
});
